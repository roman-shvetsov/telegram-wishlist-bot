...
async def update_prices(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for gift in wishlist:
        try:
            ...
            title = product_info.get('title', 'Название не найдено')
            price = product_info.get('price', 'Цена не указана')
            domain = product_info.get('domain', '')
            pool = get_pool()
            async with pool.acquire() as conn:
                await conn.execute('''
                    UPDATE wishlist 
...
async def handle_user_shared(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            f"Не удалось отправить запрос. Попросите друга отправить /start или проверить, не заблокирован ли бот.",
            reply_markup=main_keyboard()
        )
